...
from .quant_layer import QuantAttention,QuantDecoderLayer,QuantMLP
...
class MyTrainer(transformers.Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if loss_type == "origin":
            # gptq_train.fully_model_gptq(model)
            return super().compute_loss(model, inputs, return_outputs)
        if loss_type == "act_recon":
            labels = inputs.pop("labels", None)
            outputs = model(**inputs)
            loss = 0.0
            for m in model.modules():
                if isinstance(m, (QuantMLP, QuantAttention,QuantDecoderLayer)):
                    loss += m.compute_recon_loss().to('cuda:0')
            # alpha=1
            # ori_logits = self.get_ori_outputs(model, inputs).logits
            # logits = outputs.logits
            # task_loss = F.mse_loss(logits, ori_logits)
            # loss=loss*500+ alpha * task_loss.to('cuda:0')
            loss=loss
