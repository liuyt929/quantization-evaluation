...
from utils.utils import Rdtype
...
class QuantLinear(nn.Linear, H):
    ...
    def __init__(
        ...
        self.weight = org_module.weight.requires_grad_(False) #frozen
    ...
    def forward(
        ...
        if self.temporary:
            ...
            if R_res is not None and self.name in ["q", "k", "v", "up", "gate"]:
                temp_weight = temp_weight.to(Rdtype) @ R_res.to(Rdtype) #2048*256 256*256
                # if self.bias is not None:
                #     temp_bias = temp_bias.to(Rdtype) @ R_res.to(Rdtype) #256*
            ...
            if R_ov is not None and self.name == "v":
                ...
                if self.bias is not None:
                    temp_b = self.bias.reshape(transposed_shape[-1] // had_dim, had_dim)           # [num_heads, head_dim]
                    temp_b = torch.bmm(temp_b.unsqueeze(1), R_ov).squeeze(1)  # [num_heads, 1, head_dim] @ [num_heads, head_dim, head_dim] -> [num_heads, head_dim]
                    temp_bias = temp_b.reshape(-1)   
            # if R_ov is not None and self.name == "v":
            #     R_ov_tensor = torch.stack(list(R_ov), dim=0).to(Rdtype)  # [H, D, D]
            #     H, D, _ = R_ov_tensor.shape

            #     orig_dtype = temp_weight.dtype
            #     out_f, in_f = temp_weight.shape

            #     W = temp_weight.view(out_f, H, D)
            #     #    - 再 permute 到 [H, D, out_f]
            #     W = W.permute(1, 2, 0)

            #     # 4) batch‐matmul：对每个 head 并行做 R_ov[h] @ W[h]
            #     #    结果仍然是 [H, D, out_f]
            #     W_rot = torch.bmm(R_ov_tensor, W)

            #     # 5) reshape 回 [out_f, H*D] 并恢复原始 dtype
            #     W_rot = W_rot.permute(2, 0, 1)               # [out_f, H, D]
            #     temp_weight = W_rot.reshape(out_f, in_f)    # [out_f, H*D]
            #     temp_weight = temp_weight.to(orig_dtype)

            #     # 如果需要同时对 bias 做相同的旋转（通常 bias 不旋转），可以同理处理：
            #     if self.bias is not None:
            #         b = self.bias.to(Rdtype).view(H, D, -1)       # e.g. [H, D, out_f]? 
            #         b_rot = torch.bmm(R_ov_tensor, b)             # ...
            #         self.bias = b_rot.reshape(out_f).to(orig_dtype)
            if R_ov is not None and self.name == "o":
                
                R_ov_tensor = torch.stack(list(R_ov), dim=0).to(Rdtype)  # [heads, D, D]
                had_dim = R_ov_tensor.shape[-1]

                
                init_shape = temp_weight.shape           # torch.Size([out_f, in_f])
                orig_dtype = temp_weight.dtype           # 保存原始 dtype
                out_f, in_f = init_shape
                num_heads = in_f // had_dim              # in_f = num_heads * had_dim

            
                temp = temp_weight.view(out_f, num_heads, had_dim)

                # 4) 用 einsum 做 batch matmul
                if self.num_key_value_groups != 1:
                    # 重复旋转矩阵
                    R_expanded = R_ov_tensor.repeat_interleave(self.num_key_value_groups, dim=0)  # [H*G, D, D]
                    temp = temp.view(out_f, -1, had_dim)                                         # [out_f, H*G, D]
                    # 对每个 out_f、每个 block 做 matmul
                    temp = torch.einsum('obd, bdk -> obk', temp.to(Rdtype), R_expanded)           # [out_f, H*G, D]
                else:
                    # 直接对 [out_f, H, D] 和 [H, D, D] 做 matmul
                    temp = torch.einsum('ohd, hdk -> ohk', temp.to(Rdtype), R_ov_tensor)          # [out_f, H, D]

                temp = temp.reshape(init_shape)
                temp_weight = temp.to(orig_dtype)
                if self.bias is not None:
                    temp_bias = self.bias
                    bias_shape = temp_bias.shape  # [out_f]
                    orig_dtype = temp_bias.dtype
                    out_f = bias_shape[0]

                    # 1. reshape 成 [out_f, num_heads, head_dim]
                    temp_bias = temp_bias.view(out_f, num_heads, had_dim)  # [out_f, H, D]
                    if self.num_key_value_groups != 1:
                        # 扩展 R_ov
                        R_expanded = R_ov_tensor.repeat_interleave(self.num_key_value_groups, dim=0)  # [H*G, D, D]
                        temp_bias = temp_bias.view(out_f, -1, had_dim)  # [out_f, H*G, D]
                        temp_bias = torch.einsum('obd, bdk -> obk', temp_bias.to(Rdtype), R_expanded)  # [out_f, H*G, D]
                    else:
                        # [out_f, H, D] x [H, D, D] → [out_f, H, D]
                        temp_bias = torch.einsum('ohd, hdk -> ohk', temp_bias.to(Rdtype), R_ov_tensor)  # [out_f, H, D]

                    temp_bias = temp_bias.reshape(bias_shape).to(orig_dtype)  # flatten 回去，恢复原来 shape
            # if R_ov is not None and self.name == "o":
            #     R_ov = torch.stack(list(R_ov), dim=0)
            #     had_dim = R_ov.shape[-1]
