...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None or self.config.rope_scaling["rope_type"]=='llama3':
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # if use_weight_quant and self.use_weight_quant:
        #     gptq_utils_old.fully_model_gptq(self)
        output_attentions = (
            output_attentions
            if output_attentions is not None
            else self.config.output_attentions
