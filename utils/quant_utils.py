...
from train_utils.quant_ops import QuantLinear
from utils import hadamard_utils
from utils.utils import HadamardTransform
# from nonuniform.quantizer import LTQ

def stochastic_round(x: torch.Tensor) -> torch.Tensor:
    # 生成 [0, 1) 的均匀分布随机数
    noise = torch.rand_like(x)
    # 随机取整逻辑：如果小数部分 > 随机数，则向上取整，否则向下取整
    return torch.floor(x) + (noise < (x - torch.floor(x))).to(x.dtype)

def stochastic_round_selective(x: torch.Tensor, low=0.4, high=0.6) -> torch.Tensor:
    floor_x = torch.floor(x)
    frac = x - floor_x
    noise = torch.rand_like(x)

    # 先默认向下取整
    result = floor_x.clone()

    # 大于 high 的部分直接 +1
    result = torch.where(frac > high, floor_x + 1, result)

    # 在 [low, high] 范围内的部分随机取整
    rand_round = (noise < frac).to(x.dtype)
    result = torch.where((frac >= low) & (frac <= high), floor_x + rand_round, result)
...
def asym_quant(x, scale, zero, maxq, low=0.4, high=0.6):
    scale = scale.to(x.device)
    zero = zero.to(x.device)
    q = torch.clamp(stochastic_round_selective(x / scale, low, high) + zero, 0, maxq)
    return q, scale, zero
...
# def asym_quant_dequant(x, scale, zero, maxq, low=0.4, high=0.6):
#     return asym_dequant(*asym_quant(x, scale, zero, maxq, low, high))


# def sym_quant(x, scale, maxq, low=0.4, high=0.6):
#     scale = scale.to(x.device)
#     q = torch.clamp(stochastic_round_selective(x / scale, low, high), -(maxq + 1), maxq)
#     return q, scale
...
# def sym_quant_dequant(x, scale, maxq, low=0.4, high=0.6):
#     return sym_dequant(*sym_quant(x, scale, maxq, low, high))


# class STEQuantize(torch.autograd.Function):
#     @staticmethod
#     def forward(ctx, x, scale, maxq, low=0.4, high=0.6):
#         scale = scale.to(x.device)
#         q = torch.clamp(stochastic_round_selective(x / scale, low, high), -(maxq + 1), maxq)
#         return scale * q
...
# class AsymSTEQuantize(torch.autograd.Function):
#     @staticmethod
#     def forward(ctx, x, scale, zero, maxq, low=0.4, high=0.6):
#         scale = scale.to(x.device)
#         zero = zero.to(x.device)
#         q = torch.clamp(stochastic_round_selective(x / scale, low, high) + zero, 0, maxq)
#         return scale * (q - zero)
...
class ActQuantizer(torch.nn.Module):
    ...
    def find_params_per_channel_groupwise(self, x) -> None:
        init_shape = x.shape

        # Reshape to separate channels and groups within each channel
        reshaped_x = x.reshape(
        -1, x.shape[1], x.shape[-2], x.shape[-1] // self.groupsize, self.groupsize
        )  # -1: batch size, x.shape[1]: channels, etc.

        # Compute the maximum and minimum values per channel and per group
        xmax = torch.amax(reshaped_x, dim=4, keepdim=True) * self.clip_ratio
        xmin = torch.amin(reshaped_x, dim=4, keepdim=True) * self.clip_ratio

        if self.sym:
            # For symmetric quantization, xmax should be the max of the absolute xmin and xmax
            xmax = torch.maximum(torch.abs(xmin), xmax)
            tmp = xmax == 0
            self.scale = xmax / self.maxq
            self.scale[tmp] = 1
            ...
            # Repeat the scale and zero values across the groups for each channel
            self.scale = self.scale.repeat(1, 1, 1, self.groupsize).reshape(init_shape)
            self.zero = self.zero.repeat(1, 1, 1, self.groupsize).reshape(init_shape)
    def find_channel_params(self, x) -> None:
        if self.bits == 16:
            return
        ...
        if self.groupsize > 0:
            # 使用 per-channel groupwise 量化
            self.find_params_per_channel_groupwise(x)
            return
...
class ActQuantWrapper(torch.nn.Module):
    ...
    def __init__(self, module: torch.nn.Linear,enable_low_rank=False,rank=32) -> None:
        ...
        if self.enable_low_rank:
            self.rank=rank 
            self.A = torch.nn.Parameter(torch.zeros(self.module.weight.shape[1], self.rank),dtype = torch.bfloat16)
            self.B = torch.nn.Parameter(torch.zeros(self.rank, self.module.weight.shape[0]),dtype = torch.bfloat16)
        # Y = X_q * W_q.T + (X_q * A_q) * B_q
        #   = X_q * (W_q.T + A_q * B_q)
    def add_low_rank(self,enable_low_rank,rank):
        self.enable_low_rank=enable_low_rank
        if self.enable_low_rank:
            self.rank=rank 
            self.A = torch.nn.Parameter(torch.zeros(self.module.weight.shape[1], self.rank,dtype = torch.bfloat16))
    ...
    def extra_repr(self) -> str:
        ...
        if self.enable_low_rank:
            str_ +=f"\nenable_low_rank: rank:{self.rank},A:{self.A.shape}"
        return str_

    def forward(self, x, **R_S_factors):
        x_dtype = x.dtype


            # print(x.device)
            # print(self.module.weight.device)
        # Rotate, if needed
        ...
        if self.quantizer.bits < 16:  # Quantize, if needed
            # x=x.permute(0, 2, 1)
            self.quantizer.find_params(x) #处理参数数据，处理group
            qx = self.quantizer(x).to(x_dtype)
            self.quantizer.free()
            # qx=qx.permute(0, 2, 1) 
        else:
            qx=x
                # x经过low-rank branch
        # if isinstance(R_S_factors, dict)
        # 
        # if qx.device != self.module.weight.device:
        # qx = qx.to(self.module.weight.device)
        x = self.module(qx, **R_S_factors)
        # elif R_S_factors is None:
        #     x = self.module(qx)
           
        if self.enable_low_rank:
            xA=torch.matmul(qx,self.A)
...
class WeightQuantizer(torch.nn.Module):
    ...
    #     if self.ready() and self.bits < 16:
    #         scale = self.scale.to(x.device)
    #         q = torch.clamp(stochastic_round_selective(x / scale), -(self.maxq + 1), self.maxq)
    #         return (scale * q).to(x_dtype), q, scale
    #     else:
    #         return None, None, None
...
def find_qlayers(
    module,
    layers=[torch.nn.Linear, ActQuantWrapper,QuantLinear],
    name: str = "",
):
