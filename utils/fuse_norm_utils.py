...
def fuse_ln_linear(
    ...
    for linear in linear_layers:
        ...
        linear.weight.data = (W_ * layernorm.weight.double().to(W_.device)).to(linear_dtype)
        ...
        if hasattr(layernorm, "bias") and layernorm.bias is not None:
            ...
            linear.bias.data = linear.bias.data.double().to(W_.device)+ torch.matmul(
                W_, layernorm.bias.double().to(W_.device)
            )
