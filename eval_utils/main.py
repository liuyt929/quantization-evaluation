...
from data_normalization import calibration,smooth_utils,rotation_utils
...
from utils import data_utils, fuse_norm_utils, hadamard_utils, utils,quant_utils,eval_utils
from utils.convert_to_executorch import (
    sanitize_checkpoint_from_spinquant,
    write_model_llama,
)

def ptq_model(args, model, tokenizer, model_args=None):
    ...
    # endregion
    # kurtosis.kurtosis_compute(model)
    # Rotate the weights
    if args.rotate or args.smooth:
        fuse_norm_utils.fuse_layer_norms(model)
    if args.rotate:
    ...
    if args.smooth: 
        model.to('cuda')
        # calibrate and store scales
        act_scales = calibration.get_act_scales(model, tokenizer, args.dataset_path, args.num_samples, args.seq_len)
        # smooth
        smooth_utils.smooth_lm(model, act_scales,args.smooth_alpha)
    # smooth_train在此之前完成
    ...
    # kurtosis.kurtosis_compute(model,True)
    # compute SVD branch
    if args.enable_low_rank and args.w_rtn:
        qlayers = quant_utils.find_qlayers(model, layers=[quant_utils.ActQuantWrapper])
        for name in tqdm(qlayers, desc="adding low_rank_branch"):
            if "lm_head" in name:  # Skip lm_head quantization
                continue
            qlayers[name].add_low_rank(True,args.rank)
            svd_utils.LayerSvdCompute.approximate(qlayers[name], name, args)
    ...
    if args.a_bits < 16 or args.v_bits < 16:
        ...
        for name in qlayers:
            ...
            if "o_proj" in name:
                ...
                if layer_groupsize==-1 or layer_groupsize>head_dim:
                    layer_groupsize = head_dim
